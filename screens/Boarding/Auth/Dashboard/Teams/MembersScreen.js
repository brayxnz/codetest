...
import {supabase} from '../../../../../CBD';import AsyncStorage from '@react-native-async-storage/async-storage';
...
export default function MembersScreen({ route }) {
  ...
  const navigation = useNavigation(); // 👈 mejor tener el hook aquí
  ...
  const loadMembers = async () => {
    ...
    const loadTeam = async () => {
      const { data, error } = await supabase
      .from('teams')
      .select('id_team, team_name, is_boosted, boosted_by_name')
      .eq('id_team', teamId)
      .single();
      ...
      if (!error && data) {
        ...
        console.log('Error cargando team:', error);
      }
    };
    if (!error && data) {
    ...
    const handleDeleteTeam = async () => {
      ...
      const handleNestBoost = async () => {
        ...
        try {
          ...
          if (error) {
            console.log('Error checkout-nestboost:', {
          ...
          console.log('Error NestBoost:', e);
          Alert.alert('Error', 'Ocurrió un problema al iniciar el pago');
        } finally {
      ...
      const renderFooter = () => (
        ...
        {isBoosted ? (
          ...
          >
          <Ionicons name="rocket-outline" size={20} color="#FF4444" />
          <View style={{ marginLeft: 10 }}>
          <Text style={styles.upgradeTitle}>Boostear equipo</Text>
          <Text style={styles.upgradeSubtitle}>
          Activa NestBoost para obtener más beneficios y almacenamiento compartido.
          </Text>
          </View>
          </TouchableOpacity>
        )}
        ...
        return (
          ...
          <Ionicons name="rocket-outline" size={26} color="#d44e00" />
          <Text style={styles.upgradeModalTitle}>Mejora este equipo con NestBoost! =P</Text>
          </View>
          
          <Text style={styles.upgradeModalSubtitle}>
          Mejora este equipo con más almacenamiento y beneficios compartidos.
          </Text>
          
          <View style={styles.upgradeBenefits}>
          <View style={styles.upgradeBenefitRow}>
          <Ionicons name="cloud-upload-outline" size={18} color="#d44e00" />
          <Text style={styles.upgradeBenefitText}>
          Almacenamiento compartido de archivos para el equipo.
          </Text>
          </View>
          <View style={styles.upgradeBenefitRow}>
          <Ionicons name="people-outline" size={18} color="#d44e00" />
          <Text style={styles.upgradeBenefitText}>
          Más flexibilidad para miembros y proyectos.
          </Text>
          </View>
          <View style={styles.upgradeBenefitRow}>
          <Ionicons name="color-wand-outline" size={18} color="#d44e00" />
          <Text style={styles.upgradeBenefitText}>
          Cosméticos y distintivos exclusivos de NestBoost.
          </Text>
          </View>
          </View>
          
          <Text style={styles.upgradePrice}>
          Solo <Text style={{ color: '#d44e00' }}>$10 MXN</Text> al mes por equipo.
          </Text>
          
          <View style={styles.upgradeButtonsRow}>
          <TouchableOpacity
          style={[styles.upgradePrimaryBtn, upgradeLoading && { opacity: 0.7 }]}
          onPress={handleNestBoost}
          disabled={upgradeLoading}
          >
          {upgradeLoading ? (
            <ActivityIndicator color="#fff" />
          ) : (
            <Text style={styles.upgradePrimaryText}>Ir a pagar</Text>
          )}
          </TouchableOpacity>
          ...
          disabled={upgradeLoading}
          >
          <Text style={styles.upgradeSecondaryText}>Cancelar</Text>
          </TouchableOpacity>
          </View>
          </View>
          </View>
